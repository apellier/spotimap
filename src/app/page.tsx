...
export default function HomePage() {
    ...
    useEffect(() => {
        if (authStatus === "loading" && !session) {
            ...
            const playlistInfo = playlists.find(p => p.id === selectedPlaylistId);
            const totalTracksInPlaylist = playlistInfo?.tracks.total || 0;
            ...
            setLoaderMessage(
                `Processing ${currentTracks.length} songs: Retrieving artist origins (${processedArtistCount}/${totalUniqueArtistsInCurrentSet} artists)...`
            );
        ...
        authStatus, session,
        isLoadingPlaylists, playlists, selectedPlaylistId, // playlists object for name and total
        isLoadingLikedSongs,
        isLoadingPlaylistTracks,
        currentTracks.length,
        isLoadingArtistCountries, artistCountries.size, totalUniqueArtistsInCurrentSet,
        isAggregating
    ]);
    ...
    useEffect(() => {
        ...
        const legendColorScale = d3.scaleLog<string, string>()
            .domain(domainPoints)
            .range(["#C7F9CC", "#1ED760", "#00441B"].slice(0, domainPoints.length > 2 ? 3 : domainPoints.length)) // Adjust range correctly
            .interpolate(d3.interpolateRgb)
            .clamp(true);

        let steps = [1];
        if (safeMaxCount > 1) { // Ensure midPoint and maxCount are only added if relevant
             if (midPoint > 1 && midPoint < safeMaxCount) {
                steps.push(midPoint);
            }
            steps.push(safeMaxCount);
        }
        steps = [...new Set(steps.filter(s => s > 0))].sort((a, b) => a - b);
         if (steps.length === 0 && safeMaxCount === 1) steps = [1]; // Handle maxCount=1 case
        ...
        if (steps.length > 0) {
            steps.forEach((step, index) => {
                ...
                if (index === steps.length - 1 && steps.length > 1 && step > (steps[index-1] || 0) ) {
                     if (steps.length === 2 && steps[0] === 1 && step > 1) labelText = "1+"; // Special for 1, X
                     else if (step > 1 && steps[index-1] > 0) labelText = `${steps[index-1]}+`;
                     else if (step === 1 && steps.length > 1) labelText = "1"; // If max is 1, just "1"
                     else labelText = `${step}`;

                } else if (steps.length === 1) { // Only one step
                     labelText = `${step}`;
                }
        ...
        const distinctLegendItemsMap = new Map<string, LegendItem>();
        newLegendItems.forEach(item => {
            if (!distinctLegendItemsMap.has(item.label.toLowerCase())) {
                distinctLegendItemsMap.set(item.label.toLowerCase(), item);
            }
        });
        setLegendItems(Array.from(distinctLegendItemsMap.values()));
    ...
    const countryPanelDataRef = useRef({ countrySongCounts, currentTracks, artistCountries });
    useEffect(() => {
        countryPanelDataRef.current = { countrySongCounts, currentTracks, artistCountries };
    ...
    const handleCountryClick = useCallback((isoCode: string, countryNameFromMap: string) => {
        const {
            countrySongCounts: currentCountrySongCountsRef,
            currentTracks: currentTracksRef,
            artistCountries: currentArtistCountriesRef
        } = countryPanelDataRef.current;

        const songCount = currentCountrySongCountsRef.get(isoCode.toUpperCase()) || 0;
        const artistsFromCountry: ArtistDetail[] = [];
        if (songCount > 0) {
            currentTracksRef.forEach(item => {
                if (item.track?.artists?.[0]?.name) {
                    const firstArtist = item.track.artists[0];
                    const artistCountry = currentArtistCountriesRef.get(firstArtist.name.toLowerCase());
    ...
    }, []); // Empty dependency array makes the function reference stable
        ...
        <div className="flex min-h-screen flex-col bg-nb-bg text-nb-text">
            ...
            <main className="flex flex-grow pt-[55px]">
                <div className="relative h-full w-full flex-grow">
                    ...
                    {legendItems.length > 0 && !loaderMessage && <MapLegend legendItems={legendItems} />}

                    {!loaderMessage && currentTracks.length === 0 && 
                     !isLoadingLikedSongs && !isLoadingPlaylistTracks && !isLoadingPlaylists && !isLoadingArtistCountries && !isAggregating && (
                        <div className="absolute inset-0 z-10 flex h-full w-full items-center justify-center border-nb-thick border-dashed border-nb-border/50 bg-nb-bg/70 p-nb-lg text-center text-nb-text/70 backdrop-blur-sm">
